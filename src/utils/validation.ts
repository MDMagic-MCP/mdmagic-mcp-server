...
export const convertDocumentSchema = z.object({
  ...
  templateName: z.string().describe("Template to use for conversion (e.g., 'Academic_Grey', 'Corporate_Navy')"),
  outputFormat: z.enum(['docx', 'pdf', 'html', 'all']).describe("Output format: docx (DOCX only), pdf (DOCX+PDF), html (DOCX+HTML), all (DOCX+PDF+HTML)"),
  pageSize: z.enum(['A3', 'A4', 'Executive', 'US_Legal', 'US_Letter']).default('A4').describe("Page size (defaults to user's account setting: A4)"),
  orientation: z.enum(['Portrait', 'Landscape']).default('Portrait').describe("Page orientation (defaults to user's account setting: Portrait)")
}).refine(
  (data) => {
